---
title: Códigos bancarios para PayOuts con transferencia
excerpt: >-
  Explora las listas de códigos bancarios para retiros por transferencia,
  dividido por país.
deprecated: false
hidden: false
metadata:
  title: Códigos bancarios para payouts con transferencia | ProntoPaga Docs
  description: >-
    To create a new bank transfer payout in Chile, Ecuador, or Peru, you must
    use the “bankCode” parameter in this endpoint. This guide provides the full
    list of bank codes required by country. 
  image: >-
    https://files.readme.io/97b0c0e71796b81eeb9fe5a4f34a1bf82a38fd2754a714ca80d0f4c66127d350-Prontopaga_logotipo.png
  keywords:
    - Prontopaga códigos bancarios
    - códigos bancarios para payouts transferencia
    - bank codes Chile
    - bank codes Ecuador
    - bank codes Peru
    - Prontopaga payouts
    - payout API Latin America
    - create bank payout
  robots: index
next:
  description: ''
---
Para crear un nuevo retiro por transferencia bancaria en Brasil, Chile, Ecuador o Perú, es necesario utilizar [este endpoint](https://docs.prontopaga.com/reference/payout), en el cual se debe enviar el código bancario en el parámetro `bankCode`. En este artículo se muestran las listas de códigos bancarios por país.

***

## Chile

```json
[
   {
      "code":1,
      "name":"Banco de Chile"
   },
   {
      "code":9,
      "name":"Banco Internacional"
   },
   {
      "code":12,
      "name":"Banco del Estado de Chile"
   },
   {
      "code":14,
      "name":"Scotiabank Chile"
   },
   {
      "code":16,
      "name":"BCI (Banco Crédito e Inversiones) - Mach"
   },
   {
      "code":28,
      "name":"Banco Bice"
   },
   {
      "code":31,
      "name":"HSBC Bank"
   },
   {
      "code":37,
      "name":"Banco Santander- Santiago"
   },
   {
      "code":39,
      "name":"Banco Itaú"
   },
   {
      "code":49,
      "name":"Banco Security"
   },
   {
      "code":51,
      "name":"Banco Falabella"
   },
   {
      "code":53,
      "name":"Banco Ripley"
   },
   {
      "code":55,
      "name":"Consorcio"
   },
   {
      "code":59,
      "name":"Banco BTG Pactual Chile"
   },
   {
      "code":62,
      "name":"Tanner Banco Digital"
   },
   {
      "code":504,
      "name":"BBVA Chile"
   },
   {
      "code":507,
      "name":"Banco del Desarrollo"
   },
   {
      "code":672,
      "name":"Coopeuch"
   },
   {
      "code":697,
      "name":"La Polar Prepago"
   },
   {
      "code":729,
      "name":"Prepago los Héroes"
   },
   {
      "code":730,
      "name":"Tenpo"
   },
   {
      "code":732,
      "name":"Tapp - Prepago los Andes - Caja los andes"
   },
   {
      "code":738,
      "name":"Global66"
   },
   {
      "code":741,
      "name":"Copec Pay"
   },
   {
      "code":743,
      "name":"Prex"
   },
   {
      "code":746,
      "name":"Fintual"
   },
   {
      "code":875,
      "name":"MercadoPago”
   }
]
```

***

## Ecuador

```json
[
   {
      "code": 1,
      "name": "BANCO CENTRAL DEL ECUADOR"
   },
   {
      "code": 2,
      "name": "FOMENTO"
   },
   {
      "code": 10,
      "name": "BANCO PICHINCHA"
   },
   {
      "code": 17,
      "name": "BANCO DE GUAYAQUIL"
   },
   {
      "code": 19,
      "name": "TERRITORIAL"
   },
   {
      "code": 20,
      "name": "LLOYDS BANK"
   },
   {
      "code": 24,
      "name": "CITIBANK"
   },
   {
      "code": 25,
      "name": "BANCO MACHALA"
   },
   {
      "code": 26,
      "name": "UNIBANCO"
   },
   {
      "code": 29,
      "name": "BANCO DE LOJA"
   },
   {
      "code": 30,
      "name": "BANCO DEL PACIFICO"
   },
   {
      "code": 32,
      "name": "BANCO INTERNACIONAL"
   },
   {
      "code": 34,
      "name": "BANCO AMAZONAS"
   },
   {
      "code": 35,
      "name": "BANCO DEL AUSTRO"
   },
   {
      "code": 36,
      "name": "PRODUBANCO y BANCO PROMERICA"
   },
   {
      "code": 37,
      "name": "BANCO BOLIVARIANO"
   },
   {
      "code": 39,
      "name": "BANCO COMERCIAL DE MANABI"
   },
   {
      "code": 42,
      "name": "BANCO GENERAL RUMINAHUI"
   },
   {
      "code": 43,
      "name": "BANCO DEL LITORAL"
   },
   {
      "code": 45,
      "name": "SUDAMERICANO"
   },
   {
      "code": 50,
      "name": "COFIEC"
   },
   {
      "code": 59,
      "name": "BANCO SOLIDARIO"
   },
   {
      "code": 60,
      "name": "BANCO PROCREDIT"
   },
   {
      "code": 62,
      "name": "BANCO CAPITAL S.A. CORFINSA"
   },
   {
      "code": 85,
      "name": "MUTUALISTA IMBABURA"
   },
   {
      "code": 86,
      "name": "MUTUALISTA PICHINCHA"
   },
   {
      "code": 87,
      "name": "MUTUALISTA AMBATO"
   },
   {
      "code": 88,
      "name": "MUTUALISTA AZUAY"
   },
   {
      "code": 140,
      "name": "ECUATORIANO DE LA VIVIENDA"
   },
   {
      "code": 9964,
      "name": "FINANCIERA FINANCOOP"
   },
   {
      "code": 9965,
      "name": "COOP. AHORRO Y CREDITO PROGRESO"
   },
   {
      "code": 9966,
      "name": "DELBANK S.A."
   },
   {
      "code": 9967,
      "name": "DINERS CLUB"
   },
   {
      "code": 9968,
      "name": "PACIFICARD"
   },
   {
      "code": 9969,
      "name": "COOP. TULCAN"
   },
   {
      "code": 9970,
      "name": "COOP. PABLO MUNOZ VEGA"
   },
   {
      "code": 9971,
      "name": "COOP. CALCETA LTDA."
   },
   {
      "code": 9972,
      "name": "COOP. CAMARA DE COMERCIO QUITO"
   },
   {
      "code": 9974,
      "name": "COOP. JUVENTUD ECUATORIANA PROGRESISTA LTDA."
   },
   {
      "code": 9975,
      "name": "COOP. AHO Y CREDITO 23 DE JULIO"
   },
   {
      "code": 9976,
      "name": "COOP. AHO Y CREDITO SANTA ANA"
   },
   {
      "code": 9977,
      "name": "COOP. ALIANZA DEL VALLE LTDA."
   },
   {
      "code": 9978,
      "name": "COOP AHO Y CREDITO DESARROLLO PUEBLOS"
   },
   {
      "code": 9979,
      "name": "COOP. RIOBAMBA"
   },
   {
      "code": 9980,
      "name": "COOP. COMERCIO LTDA PORTOVIEJO"
   },
   {
      "code": 9981,
      "name": "COOP. CHONE LTDA."
   },
   {
      "code": 9982,
      "name": "COOP. CACPECO"
   },
   {
      "code": 9983,
      "name": "COOP. ATUNTAQUI"
   },
   {
      "code": 9984,
      "name": "COOP. GUARANDA"
   },
   {
      "code": 9985,
      "name": "COOP AHO Y CREDITO SANTA ROSA"
   },
   {
      "code": 9986,
      "name": "COOP. AHO Y CREDITO EL SAGRARIO"
   },
   {
      "code": 9987,
      "name": "COOP. OSCUS"
   },
   {
      "code": 9988,
      "name": "COOP. LA DOLOROSA"
   },
   {
      "code": 9989,
      "name": "COOP AHO Y CREDITO  MANUEL GODOY"
   },
   {
      "code": 9990,
      "name": "COOP. AHO Y CREDITO NACIONAL"
   },
   {
      "code": 9991,
      "name": "COOP AHO Y CREDITO SAN JOSE"
   },
   {
      "code": 9993,
      "name": "COOP. AHO Y CREDITO JARDIN AZUAYO"
   },
   {
      "code": 9994,
      "name": "COOP. COTOCOLLAO"
   },
   {
      "code": 9995,
      "name": "COOP. 29 DE OCTUBRE"
   },
   {
      "code": 9997,
      "name": "COOP. PEQ. EMPRESA DE PASTAZA"
   },
   {
      "code": 9998,
      "name": "COOP. ANDALUCIA"
   },
   {
      "code": 9999,
      "name": "COOP. PREVISION AHORRO Y DESARROLLO"
   }
]
```

***

## Perú

```json
[
   {
      "code":002,
      "name":"Banco de Crédito del Perú"
   },
   {
      "code":003,
      "name":"Interbank"
   },
   {
      "code":007,
      "name":"Citibank"
   },
   {
      "code":009,
      "name":"Scotiabank"
   },
   {
      "code":011,
      "name":"BBVA"
   },
   {
      "code":018,
      "name":"Banco de la Nación"
   },
   {
      "code":023,
      "name":"Banco de Comercio (Bancom)"
   },
   {
      "code":035,
      "name":"Banco Pichincha"
   },
   {
      "code":038,
      "name":"Banbif"
   },
   {
      "code":049,
      "name":"Mi Banco"
   },
   {
      "code":053,
      "name":"Banco GNB"
   },
   {
      "code":054,
      "name":"Banco Falabella"
   },
   {
      "code":055,
      "name":"Banco Ripley"
   },
   {
      "code":056,
      "name":"Santander"
   },
   {
      "code":058,
      "name":"Banco Alfin"
   },
   {
      "code":073,
      "name":"Credinka"
   },
   {
      "code":091,
      "name":"Compartamos Financiera"
   },
   {
      "code":094,
      "name":"Tarjeta oh"
   },
   {
      "code":096,
      "name":"Financiera Efectiva"
   },
   {
      "code":099,
      "name":"Financiera Confianza"
   },
   {
      "code":775,
      "name":"Luqea"
   },
   {
      "code":776,
      "name":"Prexpe"
   },
   {
      "code":800,
      "name":"Caja Metropolitana de Lima"
   },
   {
      "code":801,
      "name":"Caja Municipal de Ahorro y Crédito Piura"
   },
   {
      "code":802,
      "name":"Caja Municipal de Ahorro y Crédito Trujillo"
   },
   {
      "code":803,
      "name":"Caja Municipal de Ahorro y Crédito Arequipa"
   },
   {
      "code":805,
      "name":"Caja Municipal de Ahorro y Crédito Sullana"
   },
   {
      "code":806,
      "name":"Wayki Caja Cusco"
   },
   {
      "code":808,
      "name":"Caja Municipal de Ahorro y Crédito Huancayo"
   },
   {
      "code":809,
      "name":"Caja Ica"
   },
   {
      "code":813,
      "name":"Caja Tacna"
   },
   {
      "code":849,
      "name":"Caja Los Andes"
   },
   {
      "code":901,
      "name":"Yape"
   },
   {
      "code":902,
      "name":"Plin"
   },
   {
      "code":903,
      "name":"CCE"
   },
   {
      "code":904,
      "name":"Bim"
   },
   {
      "code":910,
      "name":"Cooperativa Abaco"
   },
   {
      "code":922,
      "name":"Dale"
   }
]
```
