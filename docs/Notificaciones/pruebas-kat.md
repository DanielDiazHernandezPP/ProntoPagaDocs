...
> This is a Stripe-hosted page for when payment-ui is stripe-hosted. View the full page at [https://docs.stripe.com/payments/checkout/managing-limited-inventory?payment-ui=stripe-hosted](https://docs.stripe.com/payments/checkout/managing-limited-inventory?payment-ui=stripe-hosted).
...
```curl
curl -X POST https://api.stripe.com/v1/checkout/sessions/{{CHECKOUTSESSION_ID}}/expire \
  -u "<<YOUR_SECRET_KEY>>:"
```

```cli
stripe checkout sessions expire {{CHECKOUTSESSION_ID}}
```
...
session = Stripe::Checkout::Session.expire('{{CHECKOUTSESSION_ID}}')
```
...
session = client.v1.checkout.sessions.expire('{{CHECKOUTSESSION_ID}}')
```
...
session = client.checkout.sessions.expire("{{CHECKOUTSESSION_ID}}")
```
...
$session = $stripe->checkout->sessions->expire('{{CHECKOUTSESSION_ID}}', []);
```
...
Session resource = Session.retrieve("{{CHECKOUTSESSION_ID}}");
...
Session session =
  client.checkout().sessions().expire("{{CHECKOUTSESSION_ID}}", params);
```
...
result, err := session.Expire("{{CHECKOUTSESSION_ID}}", params)
```
...
params := &stripe.CheckoutSessionExpireParams{
  Session: stripe.String("{{CHECKOUTSESSION_ID}}"),
}
...
Stripe.Checkout.Session session = service.Expire("{{CHECKOUTSESSION_ID}}");
```
...
curl https://api.stripe.com/v1/checkout/sessions \
  ...
  -d mode=payment \
  -d success_url="https://example.com/success" \-d expires_at="{{NOW_PLUS_TWO_HOURS}}"
```
...
Stripe::Checkout::Session.create({
  ...
  mode: 'payment',
  success_url: 'https://example.com/success',expires_at: Time.now.to_i + (3600 * 2), # Configured to expire after 2 hours
})
```
...
stripe.checkout.Session.create(
  ...
  mode='payment',
  success_url='https://example.com/success',expires_at=int(time.time() + (3600 * 2)), # Configured to expire after 2 hours
)
```
...
$stripe->checkout->sessions->create([
  ...
  'mode' => 'payment',
  'success_url' => 'https://example.com/success','expires_at' => time() + (3600 * 2), // Configured to expire after 2 hours
]);
```
...
SessionCreateParams params =
  SessionCreateParams.builder()
    ...
    .setSuccessUrl("https://example.com/success")// Configured to expire after 2 hours
    .setExpiresAt(Instant.now().plus(2, ChronoUnit.HOURS).getEpochSecond())
    .build();
...
const session = await stripe.checkout.sessions.create({
  ...
  mode: 'payment',
  success_url: 'https://example.com/success',expires_at: Math.floor(Date.now() / 1000) + (3600 * 2), // Configured to expire after 2 hours
});
```
...
params := &stripe.CheckoutSessionParams{
  ...
  SuccessURL: stripe.String("https://example.com/success"),// Configured to expire after 2 hours
  ExpiresAt: stripe.Int64(time.Now().Add(time.Hour * 2).Unix()),
};
...
var options = new Stripe.Checkout.SessionCreateOptions
{
  ...
  Mode = "payment",
  SuccessUrl = "https://example.com/success",ExpiresAt = DateTime.UtcNow + new TimeSpan(2, 0, 0) // Configured to expire after 2 hours
};
var service = new Stripe.Checkout.SessionService();
...
curl https://api.stripe.com/v1/checkout/sessions \
  ...
  -d mode=payment \
  -d ui_mode=embedded \
  -d return_url="https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}" \-d expires_at="{{NOW_PLUS_TWO_HOURS}}"
```
...
Stripe::Checkout::Session.create({
  ...
  mode: 'payment',
  ui_mode: 'embedded',
  return_url: 'https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}',expires_at: Time.now.to_i + (3600 * 2), # Configured to expire after 2 hours
})
```
...
stripe.checkout.Session.create(
  ...
  mode='payment',
  ui_mode='embedded',
  return_url='https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}',expires_at=int(time.time() + (3600 * 2)), # Configured to expire after 2 hours
)
```
...
$stripe->checkout->sessions->create([
  ...
  'mode' => 'payment',
  'ui_mode' => 'embedded',
  'return_url' => 'https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}','expires_at' => time() + (3600 * 2), // Configured to expire after 2 hours
]);
```
...
SessionCreateParams params =
  SessionCreateParams.builder()
    ...
    .setReturnUrl("https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}")// Configured to expire after 2 hours
    .setExpiresAt(Instant.now().plus(2, ChronoUnit.HOURS).getEpochSecond())
    .build();
...
const session = await stripe.checkout.sessions.create({
  ...
  mode: 'payment',
  ui_mode: 'embedded',
  return_url: 'https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}',expires_at: Math.floor(Date.now() / 1000) + (3600 * 2), // Configured to expire after 2 hours
});
```
...
params := &stripe.CheckoutSessionParams{
  ...
  ReturnURL: stripe.String("https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}"),// Configured to expire after 2 hours
  ExpiresAt: stripe.Int64(time.Now().Add(time.Hour * 2).Unix()),
};
result, err := session.New(params);
```
...
var options = new Stripe.Checkout.SessionCreateOptions
{
  ...
  Mode = "payment",
  UiMode = "embedded",
  ReturnUrl = "https://example.com/checkout/return?session_id={CHECKOUT_SESSION_ID}",ExpiresAt = DateTime.UtcNow + new TimeSpan(2, 0, 0) // Configured to expire after 2 hours
};
var service = new Stripe.Checkout.SessionService();
service.Create(options);
```
...
> This is a Embedded components for when payment-ui is embedded-components. View the full page at [https://docs.stripe.com/payments/checkout/managing-limited-inventory?payment-ui=embedded-components](https://docs.stripe.com/payments/checkout/managing-limited-inventory?payment-ui=embedded-components).
